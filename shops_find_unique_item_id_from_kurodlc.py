#!/usr/bin/env python3
"""
shops_find_unique_item_id_from_kurodlc.py - v2.2

Extract unique item IDs from .kurodlc.json files with template generation support.

Version History:
  v2.2 (2026-02-14) - Shop-only file support
    - Added support for shop-only .kurodlc.json files (e.g. Daybreak2CostumeShop)
    - Files with only ShopItem section are now valid input
    - Updated validation to detect both full DLC and shop-only variants
...
def is_valid_kurodlc_structure(data: Dict) -> bool:
    ...
    Accepts two variants:
      1. Full DLC: Must have CostumeParam (with item_id) and DLCTableData (with items).
         Optional sections: ItemTableData, ShopItem
      2. Shop-only: Must have ShopItem (with item_id and shop_id).
         No other sections required.
    """
    if not isinstance(data, dict):
        return False
    
    # Known kurodlc section names
    known_sections = {"CostumeParam", "DLCTableData", "ItemTableData", "ShopItem"}
    
    # Check if file has at least one known section
    if not any(k in data for k in known_sections):
        return False
    
    # Variant 1: Full DLC structure (has CostumeParam and/or DLCTableData)
    has_full_dlc_sections = "CostumeParam" in data or "DLCTableData" in data
    if has_full_dlc_sections:
    ...
    # Variant 2: Shop-only structure (only ShopItem section)
    if "ShopItem" in data:
        if not isinstance(data["ShopItem"], list) or len(data["ShopItem"]) == 0:
            return False
        # Validate ShopItem entries have item_id and shop_id
        if not any(isinstance(x, dict) and "item_id" in x and "shop_id" in x 
                   for x in data["ShopItem"]):
            return False
        return True
