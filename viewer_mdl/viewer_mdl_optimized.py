...
def generate_html_content(mdl_path: Path, meshes: list) -> str:
    """Generate HTML content with compressed mesh data and full UI from viewer_mdl.py."""
...
<body>
  ...
  <script>
    ...
    var compressedData = {json.dumps(meshes_data)};
    ...
    var data = compressedData.map(function(m) {{
      var verts = new Float32Array(decompressData(m.vertices).buffer);
      var norms = new Float32Array(decompressData(m.normals).buffer);
      var idxs = new Uint32Array(decompressData(m.indices).buffer);
      return {{
        name: m.name,
    ...
    function finishScreenshot(dataURL) {{
      if (window.pywebview && window.pywebview.api) {{
        ...
        var filename = '{mdl_stem}_' + timestamp + '.png';
